...
#------ Preparation of the simulation data / initial set-ups
  ...
  x1<-x2<-x3<-x4<-x5<-x6<-x7<-a<-y<-Y1<-Y0<-Pi0<-Pi1<-p0<-p1<-Y1M0<-p10<-Pi10<-NULL
  ...
  for(j in 1:10){
    xPAR[[j]] <- list(p0=matrix(rep(0.5,2), nrow=1), p1=matrix(rep(0.5,2),nrow=1),  p2.mu=0, p2.sig=1,   p3.mu=0, p3.sig=1, p4.mu=0, p4.sig=1, p5.mu=0, p5.sig=1, p6.mu=0, p6.sig=1, p7.mu=0, p7.sig=1)
  }
  ...
  source("function_extra_covarates.R")
...
#------ posterior samples of causal effects
    EY11 <- EY00 <- NULL
    ...
    x6.var <- rinvchisq(1, nu0, tau0) # p2: # of continuous X, nu0: dfs (2), tau0: scale (2)
    x6.mu <- rnorm(1, mean(x6), sqrt(x6.var/c0)) # c0: 0.5
    
    x7.var <- rinvchisq(1, nu0, tau0) # p2: # of continuous X, nu0: dfs (2), tau0: scale (2)
    x7.mu <- rnorm(1, mean(x7), sqrt(x7.var/c0)) # c0: 0.5
    ...
    E[[l]] <- list(EY11=EY11,EY00=EY00, Ey = EY11*a+(1-a)*EY00)
