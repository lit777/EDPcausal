...
library(truncnorm)
library(dplyr)
sourceCpp("clustering_large.cpp", rebuild=T)
source("function_large.R")


y <- a <- Y1 <- Y0 <- Pi0 <- Pi1 <- p0 <- p1 <- Y1M0 <- p10 <- Pi10 <- NULL


for(process in 1:400){
  tryCatch({
...
yPAR_sig <- rep(1, 5) # sigma prior = gamma(shape=0.1, rate=10)
...
# (hyper-)parameters
Gamma_0 <- 1
a0 <- 1; b0 <- 1 # gamma prior G(1,1) for alpha_theta and alpha_omega (mean=1)
alpha_theta <- 2
alpha_omega <- 2
c0 <- 1
nu0 <- 2; tau0 <- 1
...
print(process)
save(E.final,E,E.y,E.pi,Y1,Y0,Pi1,Pi0,p1,p0, file=paste("out_edp_v1_large_",process,".RData",sep=""))
  }, error=function(e){})
}
