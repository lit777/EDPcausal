...
update_parameters <- function(Sx, Sy, xPAR_p0, xPAR_p1, xPAR_mu, xPAR_sig, yPAR_beta, yPAR_r, yPAR_sig){
  ...
  for(j in 1:length(uniqueY)){
    ...
    num <- sum(log(expit(yPAR_r[,j], X_lj)*identity(subset(y, Sy==j)==0)+identity(subset(y, Sy==j)!=0)*(1-expit(yPAR_r[,j], X_lj))*dtruncnorm(subset(y, Sy==j), a=0, b=Inf, mean=X_lj%*%yPAR_beta[,j], sd=sqrt(prop)) ))+invgamma::dinvgamma(prop, 1,1,log=TRUE) + dgamma(yPAR_sig[j,1],10000*prop,10000,log=TRUE)
    den <- sum(log(expit(yPAR_r[,j], X_lj)*identity(subset(y, Sy==j)==0)+identity(subset(y, Sy==j)!=0)*(1-expit(yPAR_r[,j], X_lj))*dtruncnorm(subset(y, Sy==j), a=0, b=Inf, mean=X_lj%*%yPAR_beta[,j], sd=sqrt(yPAR_sig[j,1])) ))+invgamma::dinvgamma(yPAR_sig[j,1], 1,1,log=TRUE) + dgamma(prop,10000*yPAR_sig[j,1],10000,log=TRUE)
