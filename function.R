  ...
  for(i in 1:n){ # the number of subjects : n
    ...
    if(lenY*lenX != 0){
      ...
      for(m in 1:M){
        ...
        xPAR.temp[[(numX+m)]] <- list(p0=x.cat0, p1=x.cat1, p2.mu=x.mu2, p2.sig=x.var2, p3.mu=x.mu3,  p3.sig=x.var3, p4.mu=x.mu4, p4.sig=x.var4, p5.mu=x.mu5, p5.sig=x.var5)
      ...
      for(m in 1:M){
        for(k in 1:numY){
          ...
          x.var5 <- rinvchisq(1, nu0, tau0) # p2: # of continuous X, nu0: dfs (2), tau0: scale (2)
          x.mu5 <- rnorm(1, 0, sqrt(x.var5/c0)) # c0: 0.5
          xPAR.temp <- rlist::list.insert(xPAR.temp, cumsum(numXj+m)[k], list(p0=x.cat0, p1=x.cat1, p2.mu=x.mu2, p2.sig=x.var2, p3.mu=x.mu3,  p3.sig=x.var3, p4.mu=x.mu4, p4.sig=x.var4, p5.mu=x.mu5, p5.sig=x.var5) )
        }
      }
    }
